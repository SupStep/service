<template>
	...
	>
		...
		<form class="form">
			...
			<n-button style="margin-right: 20px" @click="startSelectedRequest"
				>В ремонте</n-button
			>
			<n-button @click="confirmSelectedRequest">Подтверждена</n-button>
		</form>
	</n-modal>
</template>
...
const find = async () => {
	isFind.value = true
	try {
		const response = await authStore.absFio(searchFio.value)
		if (response.success) {
...
const confirmSelectedRequest = async () => {
	try {
		const response = await authStore.confirmRequest(
			selectedRequest.value.id_request
		)
		if (response.success) {
			location.reload()
...
onMounted(async () => {
	try {
		const response = await authStore.getAbsAllRequests()
		if (response.success) {
